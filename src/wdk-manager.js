...
import { DualSignatureProtocol } from './protocols/dual-signature-protocol.js'
...
export default class WDK {
  ...
  constructor (seed) {
    ...
    /** @private */
    this._protocols = { swap: { }, bridge: { }, lending: { }, dualSignature: { } }
   ...
   * @template {typeof SwapProtocol | typeof BridgeProtocol | typeof LendingProtocol | typeof DualSignatureProtocol} P
  ...
  _registerProtocols (account, { blockchain }) {
    const protocols = { swap: { }, bridge: { }, lending: { }, dualSignature: { } }
    ...
    account.getDualSignatureProtocol = (label) => {
      if (this._protocols.dualSignature[blockchain]?.[label]) {
        const { Protocol, config } = this._protocols.dualSignature[blockchain][label]
      ...
      if (protocols.dualSignature[label]) {
        return protocols.dualSignature[label]
      }

      throw new Error(`No dual signature protocol registered for label: ${label}.`)
    }
  }
}
